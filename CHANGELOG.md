...
### Fixed
- `herd unsecure` no longer hangs waiting for a password on unmount — runs in the foreground so macOS can prompt for credentials instead of being silenced inside a background spinner subprocess.
...
### Changed
- README "Starship prompt indicator" section replaced with "Shell prompt integration" covering Starship, Powerlevel10k, Oh My Zsh, and plain Bash/PS1.
...
### Added
- `wt-link starship` subcommand — outputs `⛓` when cwd contains `.worktree-link-state` (for Starship custom module).
- Starship prompt snippet documented in README.
